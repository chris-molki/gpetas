...
def plot_slice_x(intensity_ensemble, X_grid=None, intensity_1_grid=None, intensity_2_grid=None,
    ...
    if X_grid is not None:
        ...
        if X_borders is None:
            X_borders = np.array([[np.min(X_grid_prime[:, 0]), np.max(X_grid_prime[:, 0])],
                                  [np.min(X_grid_prime[:, 1]), np.max(X_grid_prime[:, 1])]])
    ...
    if xidx is not None:
        ...
        #                                             X_grid=X_grid)
        ax = h3_where.gca()
        ax.axvline(x=(xidx[0]) * dx + data_obj.domain.X_borders[0, 0], color='w', linestyle='--')
...
# stability issues
def n_mol(K, c, p, t_start=0., t_end=np.inf):
    PHI_t = np.nan
